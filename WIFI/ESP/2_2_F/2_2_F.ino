...
const char* deviceName = "GaitMelt Device FSR 1"; // Nombre del dispositivo
#define MOTORIN 26
#define FSR_FRONTAL 34
#define FSR_TRASERO 35
...
// Lectura datos del MPU6050
void readData(struct_message *data) {
  ...
  data->gyr_y = g.gyro.y;
  data->gyr_z = g.gyro.z;
  data->fsr_frontal = analogRead(FSR_FRONTAL);
  data->fsr_trasero = analogRead(FSR_TRASERO);
...
// Función para enviar vibración comprobando la conexión al WiFi
void connectionSignal(int delay_time){
...
// Función para iniciar conexión WiFi
void setupWIFI() {
...
void setup() {
  ...
  analogWrite(MOTORIN, motorPower);
  Serial.begin(115200);
  pinMode(FSR_FRONTAL, INPUT);
  pinMode(FSR_TRASERO, INPUT);
  setupMPU();
  setupWIFI();
  startTime = millis();
}
...
void loop() {
  ...
  // Enviar datos del MPU y FSR
  readData(&dataReadings);
  uint8_t buffer[sizeof(dataReadings)];
  memcpy(buffer, &dataReadings, sizeof(dataReadings));
  ...
  if (packetSize) {
      ...
      // Comprobamos si el paquete es "reset" para resetear el tiempo de ejecución
      if (strcmp(incomingPacket, "reset") == 0) {
          startTime = millis();
      }
      // Comprobamos si el paquete es "motor" para encender el motor
